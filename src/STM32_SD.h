...
//    SDFSConfig(void* peripheral=SPI3,SPISettings spi = SPISettings(SPI_FULL_SPEED, MSBFIRST, SPI_MODE0)) : FSConfig(FSId, false),  _spiSettings(spi), _peripheral(peripheral)  { }
    SDFSConfig(void* peripheral=SPI3,SPISettings spi = SPI_FULL_SPEED) : FSConfig(FSId, false),  _spiSettings(spi), _peripheral(peripheral)  { }
...
public:
    ...
    bool begin() override {
        if (_mounted) end();
        Serial.printf("x=%08x\n",(void*) _cfg._peripheral);

        PinName x;
        SPI.setMOSI(pinmap_pin(_cfg._peripheral,PinMap_SPI_MOSI));
        x=pinmap_pin(_cfg._peripheral,PinMap_SPI_MOSI);
        Serial.printf("MOSI=%d or D%d\n",x,pinNametoDigitalPin(x));
        SPI.setMISO(pinmap_pin(_cfg._peripheral,PinMap_SPI_MISO));
        x=pinmap_pin(_cfg._peripheral,PinMap_SPI_MISO);
        Serial.printf("MISO=%d or D%d\n",x,pinNametoDigitalPin(x));
        SPI.setSCLK(pinmap_pin(_cfg._peripheral,PinMap_SPI_SCLK));
        x=pinmap_pin(_cfg._peripheral,PinMap_SPI_SCLK);
        Serial.printf("SCLK=%d or D%d\n",x,pinNametoDigitalPin(x));
        x=pinmap_pin(_cfg._peripheral,PinMap_SPI_SSEL);
        
        uint8_t _csPin= pinNametoDigitalPin(x);
        Serial.printf("x=%d CS=%d\n",x,_csPin);

        SPI.setSSEL(NC);

        _mounted = _fs.begin(_csPin, _cfg._spiSettings);
...
{
public:
    SDFSFileImpl(SDFSImpl *fs, std::shared_ptr<sdfat::File> fd, const char *name): _fs(fs), _fd(fd), _opened(true){
